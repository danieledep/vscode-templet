...
- Add a marketplace icon, generated by `scripts/make-icon.mjs` so its colours and proportions can be retuned without a design tool.
- Add `galleryBanner`, `homepage` and `bugs` to the manifest for the marketplace listing.
